...
void Speaker::begin() {
    ...
    Serial.println("Hardware DAC Initialized on A0 (10-bit mode, 16kHz interrupt)");
    beginTimer();
    Serial.println("16kHz Speaker Timer initialized");
}
...
void Speaker::isr_playNextSample() {
    if (_count == 0) {
        // Output DC midpoint (1.65V) during underflow to eliminate high-voltage static spikes
        while (DAC->STATUS.bit.SYNCBUSY);
        DAC->DATA.reg = 0; 
        return; 
    }
