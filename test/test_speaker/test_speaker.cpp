...
// Static instance of the Speaker driver
Speaker speaker;

// Pre-computed 8-point lookup table for a 1 kHz sine wave at 8 kHz sample rate
static const int16_t SINE_WAVE_1KHZ[8] = {
        0,   // sin(0°)
    23170,   // sin(45°)  ~ 32767 * 0.7071
    32767,   // sin(90°)  ~ 32767 * 1.0
    23170,   // sin(135°) ~ 32767 * 0.7071
        0,   // sin(180°)
   -23170,   // sin(225°) ~ -32767 * 0.7071
   -32768,   // sin(270°) ~ -32767 * 1.0
   -23170    // sin(315°) ~ -32767 * 0.7071
};
...
void test_speaker_initialization(void) {
    speaker.begin();
    TEST_PASS();
}

void setup() {
    // Wait for Serial monitor connection (SAMD boards)
    delay(2000);

    UNITY_BEGIN();
    RUN_TEST(test_speaker_initialization);
}

void loop() {
    static uint8_t sample_index = 0;

    // Continuously attempt to queue the next sine wave sample.
    // queueAudio returns false when the ring buffer is full,
    // avoiding buffer overflows while keeping the audio buffer saturated.
    if (speaker.queueAudio(SINE_WAVE_1KHZ[sample_index])) {
